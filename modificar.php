...
if (isset($_POST['modificar'])) {
		...
		if ($_SESSION['propiedad']->validar()) {
			...
			$c->modificar($_SESSION['propiedad']);
...
echo menu_mod($_SESSION['propiedad']->get_idpropiedad(),1);
...
echo '<form action="modificar.php" id="customForm" method="POST">';
// Nombre
echo '<div>';
$c="";
...
echo '<div>';
$st="";
if ($_SESSION['propiedad']->get_errores(1)!="") {
	$info= '<span id="checkInfo">'.$_SESSION['propiedad']->get_errores(1)." ".$_SESSION['propiedad']->get_errores(2).'</span>';
	$c="error";
	$st='style="color:#dd0000;"';
}
else {
	$info= '<span id="checkInfo"></span>';
}
echo '<label for="name" '.$st.' >Operación</label>';
...
echo '<input style="width:15px;" id="operacion1" type="checkbox" name="operacion1" value="Alquiler" '.$check1.' onchange="checkOp();animatedcollapse.toggle(\'Alquiler\');" /> Alquiler';
echo '<input style="width:15px;" id="operacion2" type="checkbox" name="operacion2" onchange="checkOp();animatedcollapse.toggle(\'Venta\');" value="Venta" '.   $check2.'/> Venta';
echo $info;
echo '</div>';
...
echo '<input style="width:15px;" type="radio" name="moneda1" value="ars" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda1(),"ars").'/> Peso | ';
echo '<input style="width:15px;" type="radio" name="moneda1" value="usd" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda1(),"usd").'/> Dólar | ';
echo '<input style="width:15px;" type="radio" name="moneda1" value="eur" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda1(),"eur").'/> Euro';
echo '</div>';
...
echo '<input style="width:15px;" type="radio" name="moneda2" value="ars" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda2(),"ars").'/> Peso | ';
echo '<input style="width:15px;" type="radio" name="moneda2" value="usd" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda2(),"usd").'/> Dólar | ';
echo '<input style="width:15px;" type="radio" name="moneda2" value="eur" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_moneda2(),"eur").'/> Euro';
echo '</div>';
...
echo '<input style="width:15px;" type="radio" name="estado" value="Disponible" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_estado(),"Disponible").'/> Disponible | ';
echo '<input style="width:15px;" type="radio" name="estado" value="Reservado" '. $_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_estado(),"Reservado"). '/> Reservado';
echo '<input style="width:15px;" type="radio" name="estado" value="Alquilado" '. $_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_estado(),"Alquilado"). '/> Alquilado';
echo '<input style="width:15px;" type="radio" name="estado" value="Vendido" '.   $_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_estado(),"Vendido").   '/> Vendido';
echo '</div>';

echo '<div style="width:200px;float:left;">';
echo '<label for="name">Visibilidad</label>';
echo '<input style="width:15px;" type="radio" name="visibilidad" value="Visible" '.$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_visibilidad(),"Visible").'/> Visible';
echo '<input style="width:15px;" type="radio" name="visibilidad" value="Oculto" ' .$_SESSION['propiedad']->get_radio($_SESSION['propiedad']->get_visibilidad(),"Oculto"). '/> Oculto';
echo "</div>";
...
echo '<input id="mapa" style="width:450px;" name="mapa" type="text" value="'.htmlspecialchars($_SESSION['propiedad']->get_mapa()).'" />';
echo '</div>';
...
echo '<div style="width:50%;margin:auto;margin-top:50px;margin-bottom:50px;">';
echo '<input style="align:center;" id="send" name="modificar" type="submit" value="Modificar"/>';
echo '</div>';
